...
const Sidebar: React.FC<SidebarProps> = ({ user, currentView, onViewChange, onLogout, siteConfig, activeConversationId, onActiveConversationIdChange }) => {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-72 bg-white border-r border-stone-200 flex-col h-screen shrink-0 relative overflow-hidden">
        <div className="flex-1 flex flex-col min-h-0 p-6 overflow-hidden">
          <div 
            onClick={() => onViewChange('landing')}
            className="flex items-center gap-3 mb-8 cursor-pointer hover:opacity-80 transition-opacity shrink-0"
          >
              {siteConfig?.logo_url ? (
                  <img 
          ...
          {user && (
            <div className="flex-1 flex flex-col min-h-0 mt-4 overflow-hidden">
              <div className="flex items-center gap-3 mb-4 shrink-0 px-2">
                ...
                <p className="text-[9px] font-black text-stone-300 uppercase tracking-[0.3em]">Lịch sử chat</p>
                <div className="h-px bg-stone-100 flex-1" />
              </div>
