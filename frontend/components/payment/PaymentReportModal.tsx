...
const PaymentReportModal: React.FC<PaymentReportModalProps> = ({ user, onClose }) => {
    ...
    const fetchPayments = async () => {
      try {
        ...
        if (res.payments && res.payments.length > 0) {
          // Default to the most recent payment ID
  ...
  const handleSubmit = async () => {
    ...
    const issueLabels: Record<string, string> = {
      no_tokens: isVi ? "Đã chuyển tiền nhưng chưa nhận được tokens" : "Transferred but tokens not credited",
      wrong_amount: isVi ? "Chuyển tiền sai số tiền / sai cú pháp" : "Transferred wrong amount / incorrect message",
      qr_issue: isVi ? "Lỗi không quét được / không hiển thị mã QR" : "QR code scanning / display issue",
      other: isVi ? "Sự cố khác" : "Other issue"
    };

    // Format description text cleanly
    const formattedDescription = `【BÁO CÁO PHÚC TRA SAI SÓT NẠP TIỀN】
• Loại sự cố: ${issueLabels[issueType] || issueType}
...
${reportNote.trim()}`;
    ...
    try {
      await api.createPaymentReport(parseInt(reportPaymentId), formattedDescription, email);
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[60] overflow-y-auto">
       <div className="bg-white rounded-[40px] p-6 md:p-8 max-w-2xl w-full relative animate-in zoom-in duration-300 shadow-2xl my-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 bg-amber-500/10 rounded-full flex items-center justify-center text-amber-800">
            ...
            <div>
              <h3 className="text-xl md:text-2xl font-serif font-bold text-amber-900 leading-tight">
                {isVi ? "📜 Sớ Phúc Tra Báo Lỗi Nạp Tiền" : "📜 Ticket Audit Payment Dispute"}
              </h3>
              <p className="text-stone-400 text-[10px] uppercase tracking-widest font-black mt-1">
                {isVi ? "Tâm nguyện của bạn sẽ được ban quản trị rà soát ngay lập tức" : "The board of admins will audit your transaction immediately"}
              </p>
            </div>
          </div>

          <div className="space-y-5">
             {/* Row 1: Invoice Selection & Issue Type */}
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                   <label className="text-[10px] font-black uppercase tracking-widest text-[#7f1d1d] block mb-2 px-1">
                     {isVi ? "Loại sự cố nạp tiền" : "Type of issue"}
                   </label>
                   <select
                     className="w-full bg-stone-50 border border-stone-200/60 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-amber-500 transition-all font-medium"
                     value={issueType}
                   ...
                   >
                     <option value="no_tokens">{isVi ? "Đã chuyển tiền nhưng chưa nhận được tokens" : "Transferred but tokens not received"}</option>
                     <option value="wrong_amount">{isVi ? "Chuyển tiền sai số tiền / sai cú pháp" : "Wrong amount or transfer content"}</option>
                     <option value="qr_issue">{isVi ? "Không quét được mã QR VietQR" : "Could not scan QR / QR loading error"}</option>
                     <option value="other">{isVi ? "Sự cố khác" : "Other billing issue"}</option>
                   </select>
                </div>
             </div>
             ...
             <div>
                ...
                <textarea 
                  rows={3}
                  placeholder={isVi ? "Ghi chú thêm thông tin chi tiết (Ví dụ: tên ngân hàng, nội dung chuyển bị sai ra sao...)" : "Provide additional details (e.g. your bank name, what went wrong during transfer...)"}
