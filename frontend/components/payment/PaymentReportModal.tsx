...
const PaymentReportModal: React.FC<PaymentReportModalProps> = ({ user, onClose, siteConfig }) => {
  ...
  // Reset initial scroll flag and count when tab changes to chat
  useEffect(() => {
    if (activeTab === 'chat') {
  ...
  useEffect(() => {
    ...
    const currentCount = supportMessages.length;
    ...
    const lastMsg = supportMessages[currentCount - 1];
    const isMyNewMessage = hasNewMessage && lastMsg && lastMsg.sender_type === 'user';

    if (isNearBottom || isMyNewMessage || isSending) {
      messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
    }
  }, [supportMessages, isSending, activeTab, hasInitialScrolled]);
      ...
      <div className="bg-white rounded-[32px] p-6 md:p-8 max-w-2xl w-full h-[650px] max-h-[90vh] flex flex-col relative animate-in zoom-in duration-300 shadow-2xl">
        ...
        <div className="flex border-b border-stone-200 mb-4 shrink-0">
          <button
            ...
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-all ${activeTab === 'report'
          ...
          <button
            ...
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-all ${activeTab === 'chat'
                ? 'border-[#7f1d1d] text-[#7f1d1d]'
                : 'border-transparent text-stone-400 hover:text-stone-600'
              }`}
          >
        ...
        {activeTab === 'report' ? (
                  ...
                  >
                    {packages.map((pkg) => (
                      <option key={pkg.id} value={pkg.id.toString()}>
                        {getTransName(pkg.name)} ({pkg.amount_vnd.toLocaleString()}đ) — {pkg.tokens.toLocaleString()} Tokens
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full bg-stone-50 border border-stone-200/60 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-amber-500 transition-all pl-10 ${hasLinkedEmail ? 'opacity-65 cursor-not-allowed bg-stone-100 text-stone-500 font-medium' : 'font-medium'
                      }`}
                    value={email}
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-stone-50 rounded-2xl border border-stone-200/60 overflow-hidden my-2">
            ...
            {/* Messages List */}
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0 chatgpt-scrollbar bg-stone-50/30">
                  ...
                  return (
                    <div key={m.id} className={`flex items-start gap-2.5 ${isUser ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`max-w-[75%] flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
                        ...
                        <div className={`px-3 py-2 rounded-2xl text-xs leading-relaxed shadow-sm font-medium ${isUser
